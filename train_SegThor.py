...
class ImageDataset(Dataset):
    def __init__(self, parameters, image_paths, data_type='train', device="cpu"):
    ...
    def __getitem__(self, index):
        img_path = self.data_dir+os.sep+self.image_paths[index]
        image = (cv2.imread(img_path, cv2.IMREAD_UNCHANGED)).astype("uint16")
        new_label_path = img_path.replace(self.data_dir, self.correct_dir)
        new_label_path = new_label_path.replace("images", "masks")
        if self.data_type == 'train':
            if os.path.isfile(new_label_path):
                mask = (cv2.imread(new_label_path, cv2.IMREAD_UNCHANGED)).astype("uint8")
            else:
                mask_path = img_path.replace("images", "noisy_masks")
        ...
        return image_torch, mask, new_label_path, clean_mask
...
def train_model(parameters, dir='model', describe=''):
    ...
    tr_dl = [line.strip() for line in lines]

    valid_path = 'valid.txt'
    with open(valid_path, 'r') as file:
        lines = file.readlines()
    val_dl = [line.strip() for line in lines]

    dataset = ImageDataset(parameters, image_paths=tr_dl, data_type='train', device=device)
    ...
    val_dataset = ImageDataset(parameters, image_paths=val_dl, data_type='valid', device=device)
    ...
    with torch.no_grad():
        class_features = Prototype.inital(model, train_loader, parameters, max_samples_per_class = [1000000, 85000, 1000000, 65000, 350000]).cuda()
        past_class_features = class_features.detach().clone()

    optimizer = torch.optim.SGD(model.parameters(), lr=parameters["lr"], momentum=parameters["momentum"])
    past_optimizer = torch.optim.SGD(past_model.parameters(), lr=parameters["lr"], momentum=parameters["momentum"])
    ...
    valid_record=[]
    correct_record=[]
    need_label_correction_dict = {0:True, 1:False, 2:False, 3:False, 4:False}
    already_label_correction_dict = {0:True, 1:False, 2:False, 3:False, 4:False}
    first_correct_dict = {0:0, 1:0, 2:0, 3:0, 4:0}
    now_epoch=0
    ...
    for epoch in tqdm(range(parameters["epochs"])):
        now_epoch+=1
        train_hist = np.zeros((parameters["class_num"], 3))
        correct_hist = np.zeros((parameters["class_num"],3))
        past_model.eval()
        past_model.load_state_dict(model.state_dict())
        past_class_features = class_features.detach().clone()
        past_optimizer.load_state_dict(optimizer.state_dict())
        ...
        with torch.no_grad():
            ...
            val_hist = np.zeros((parameters["class_num"], 3))
                ...
                with torch.no_grad():
                    ...
                    for c in range(1,parameters["class_num"]):
                        if need_label_correction_dict[c]:
                            for c2 in range(parameters['class_num']):
                                if (need_label_correction_dict[c2]) & (c!=c2):
                                    label_belong_correct = (masks==c2)
                                    after_belong = (pred == c)
                                    confident = Prototype.dist_mask(total_prob, past_class_features, features1, c, parameters)
                                    mask |= (confident & label_belong_correct & after_belong)
                                    if c2==0:
                                        label_belong_correct = (masks==c)
                                        after_belong = (pred == c2)
                                        confident = Prototype.dist_mask(total_prob, past_class_features, features1, c2, parameters)
                                        mask |= ((confident) & label_belong_correct & after_belong)
                    ...
                    for i in range(mask_shape[0]):
                        array = (new_labels[i].cpu().numpy()).astype(np.uint8)
