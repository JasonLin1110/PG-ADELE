...
parser.add_argument("--data_dir", type=str, default="ISIC_2017")
parser.add_argument("--model_dir", type=str, default="PGADELE_ISIC")
...
test_dir = parameters["data_dir"]+os.sep+'Test_resize'
ts_dl = sorted([f for f in os.listdir(test_dir) if f.endswith('.jpg')])
ts_dataset = ImageDataset(parameters, image_dir = test_dir, image_paths=ts_dl, data_type='valid', device=device)
...
bestmodel_a = unet_model.UNet(n_channels=parameters["channel"], n_classes=parameters["class_num"])
...
print("IoU:  ", all_iou1)
print('mIoU: ', miou1)
print('DSC:  ', np.nanmean(2*all_iou1/(1+all_iou1)))
print('Best Epoch test data')
print("IoU:  ", all_iou2)
print('mIoU: ', miou2)
print('DSC:  ', np.nanmean(2*all_iou2/(1+all_iou2)))